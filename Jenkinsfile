    ...
    stages {
        stage('SCM_Checkout') {
            steps {
				...
				git 'https://github.com/AnupamVerma02/star-agile-banking-finance.git'
            }
        }
        ...
        stage('Build Docker Image') {
            steps {
				sh 'docker version'
				sh "docker build -t anupamverma24/bankapp-eta-app1:V${BUILD_NUMBER} ."
				sh 'docker image list'
				//sh "docker tag anupamverma24/bankapp-eta-app1:V${BUILD_NUMBER} anupamverma24/bankapp-eta-app1:latest"
            }
              post {
                success {
                  sh "echo 'Send mail docker Build Success'"
                  mail to:"shivarpit9001@gmail.com", from: 'shivarpit9001@gmail.com', subject:"App Image Created Please validate", body: "App Image Created Please validate - anupamverma24/bankapp-eta-app1:V${BUILD_NUMBER}"
                }
                failure {
		...
		stage('Publish_to_Docker_Registry') {
			steps {
				sh "docker push anupamverma24/bankapp-eta-app1:V${BUILD_NUMBER}"
			}
		}
